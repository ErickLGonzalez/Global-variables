# Observational Decompilation of Physics — Constant Atlas & Restraint Program v0.2
...
## 1. Constant Atlas v0.2 — dependency classes
...
| G (gravity) | **H** (diffeo) | P → **?₂** (GSL) | — | **H** | **?₃** (entropy additivity) | **?₄** (asympt. safety?) | P (Euler/GHY terms) | **H** (S=A/4ℓ_P²) |
| k_B | P | **H** (entropy ≥ 0) | — | — | **H** (extensivity/subadd.) | — | — | **H** |
| α (QED) | **H** (U(1)) | P (spectral fn ≥ 0) | **H** | **H** (Kramers–Kronig) | **?₅** (cluster decomp. → H) | **H** (running) | P (Dirac quantization) | P |
| α_s (QCD) | **H** (SU(3)) | P | **H** | **H** | **?₅** | **H** (asympt. freedom) | **H** (instantons, θ-sectors) | P (confinement/deconf.) |
| Electroweak block (g, g′, v) | **H** | P | **H** | **H** | **?₅** | **H** | P (sphalerons) | P |
...
| ?₂ | G positivity → H | Generalized Second Law + quantum focusing (QNEC) supply operator-level positivity (⟨T_kk⟩ bounded by entropy derivatives). | Check QNEC-type inequality as a Gram-positivity condition in a solvable model (free fields, 2D CFT — already proven there → upgrade path exists). |
| ?₃ | G composition rule | If S = A/4ℓ_P² is fundamental, merging subsystems must satisfy entropy subadditivity ↔ horizon-area theorems. Composition column of gravity = area theorem. | Black-hole merger data (LIGO/Virgo): A_final ≥ A₁ + A₂ is *already confirmed* — promote to P/H and treat as the first cross-domain composition law recovered by the atlas. |
...
| ?₅ | Composition column of all gauge couplings → H | Cluster decomposition + associativity of ⊗ are theorems in axiomatic QFT; the couplings inherit them. This is the *least* speculative blank: mostly bookkeeping. | Verify factorization of multi-sector amplitudes in the benchmark suite; failure would falsify the framework outright. |
...
| B4 | BH thermodynamics: LIGO merger catalog → verify area theorem as composition law (?₃) | First cross-domain blank-fill promoted from ? to P | Empirical composition-law report |

Then: reflection-positivity module on Euclidean lattice data; EFT positivity bounds (Adams et al.) as gate #6 test; then flavor/topology search (?₆).
