# Observational Decompilation of Physics — Constant Atlas & Restraint Program v0.4
...
**Changelog:** see §9. v0.3 filled G Cmp (?₃ → P). v0.4 fills gauge Cmp (?₅ → H) via B5 cluster/factorization certificate.
...
## 1. Constant Atlas v0.4 — dependency classes
...
Then: ?₂ QNEC/Schur reformulation; reflection-positivity module on Euclidean lattice data; EFT positivity bounds (Adams et al.) as gate #6 test; then flavor/topology search (?₆).
...
- B1 / B2 / B3 / B4 / B5 — **implemented** with certificates; next: ?₂.
