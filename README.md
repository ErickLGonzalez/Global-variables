...
**Stage 2 (started) — B9 lowering:** [`pir/domains/b9.py`](pir/domains/b9.py) lowers the committed B9 certificate into PIR L0/L1/L2 and **re-derives** B9's per-test verdicts from the stored residuals (the FDT gate at 0.15, reproduced not copied); B9's `m_layer_stipulations` become assumption-taint, so invalidating `asm:hard_wall_truncation` downgrades exactly the T6 held-out fact. Run: `python3 tests/test_pir_b9_lowering.py`. No B9 code, verdict, certificate, or atlas cell is changed.
